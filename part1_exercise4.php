...
<button class="nav_btn open_btn">
  ...
  <h1>Reverse Case</h1>
  <p>Program receives a string input, then reverse the case value and arrangement.</p>
  <ol>
    <li>abCD = dcBA, qWeTy = YtEwQ</li>
  </ol>

  <?php
    function reverse_case($string) {
      return strrev(strtolower($string) ^ strtoupper($string) ^ $string);
    }
   ?>

  <form action="part1_exercise4.php" method="post" autocomplete="off">
    <input type="text" name="string" placeholder="Enter string..." required>
    <input type="submit">
  </form>
  <div class="output_display">
    <?php
      echo isset($_POST["string"]) ? $_POST["string"]." = ".reverse_case($_POST["string"]) : " ";
    ?>
  </div>
</body>
</html>
